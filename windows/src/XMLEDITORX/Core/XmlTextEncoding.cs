...
public readonly record struct XmlTextEncoding(XmlEncodingKind Kind)
{
    ...
    public static (string Text, XmlTextEncoding Encoding) Decode(byte[] data)
    {
        ...
        if (declared is not null)
        {
            XmlTextEncoding selected = FromDeclaredName(declared, data)
                ?? throw new XmlDocumentException($"The XML declaration names unsupported encoding '{declared}'. XMLEDITORX did not guess or change the document's bytes.");
    ...
    public XmlTextEncoding ReconciledForSave(string text)
    {
        ...
        if (declared is not null)
        {
            ...
            return selected ?? throw new XmlDocumentException($"The XML declaration names unsupported encoding '{declared}'. XMLEDITORX did not guess or change the document's bytes.");
        }
