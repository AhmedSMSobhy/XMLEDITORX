...
public partial class MainWindow
{
    ...
    private void SharePrintSelection()
    {
        ...
        string job = _currentFileUrl is not null ? FileNameOf(_currentFileUrl) : "XMLEDITORX";
    ...
    private void ShareXmlText(string xml, LLMTarget target, Func<string, string> makePrompt)
    {
        ...
        while (limit >= ShareTextPolicy.CompatibilityUtf8ByteLimit)
        {
            ...
            string prompt = makePrompt(excerpt.Text)
                + $"\n\nXMLEDITORX excerpt: {excerpt.IncludedUtf8Bytes:N0} of {excerpt.TotalUtf8Bytes:N0} UTF-8 bytes "
    ...
    private void ShareCopySelectionInFull()
    {
        ...
        SetStatus($"Copied the complete selection ({bytes:N0} UTF-8 bytes); XMLEDITORX applied no share limit");
    }
    ...
    private void ShowFileWorkInProgress()
    {
        ...
        ShowAlert("Please wait for the file operation",
            "XMLEDITORX is still opening or saving a file. The window will stay open until that operation finishes.",
            new[] { "OK" });
    }
