...
final class MainWindowController: NSWindowController, NSWindowDelegate, NSSplitViewDelegate {
    ...
    private var toolbarHelper: XMLEDITORXToolbar?
    ...
    static let minimapVisibleKey = "XMLEDITORX.minimapVisible"
    static var minimapVisible: Bool {
        let d = UserDefaults.standard
        return d.object(forKey: minimapVisibleKey) == nil ? true : d.bool(forKey: minimapVisibleKey)
    }
    private static let detailSelectionKey = "XMLEDITORX.activeDetailSection"
    ...
    override init(window: NSWindow?) {
        ...
        win.title = "XMLEDITORX v\(version)"
        win.center()
        ...
        win.setFrameAutosaveName("XMLEDITORXMainWindow")
        ...
        win.delegate = self
        let helper = XMLEDITORXToolbar(target: self)
    ...
    private func buildLayout() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let savedZoom = UserDefaults.standard.double(forKey: "XMLEDITORX.globalZoom")
            ...
            DispatchQueue.main.async {
                ...
                guard self.fileFingerprint(at: url) == openingFingerprint else {
                    self.handleOpenFailure(
                        sessionID: sessionID,
                        url: url,
                        message: "The file changed while XMLEDITORX was parsing it. Open it again after the other write finishes."
                    )
                    return
                }
                ...
                self.window?.title = "\(url.lastPathComponent), XMLEDITORX v\(v)"
                ...
                self.sourceVC.onFileLoaded = { [weak self] in
                    ...
                    guard self.fileFingerprint(at: url) == openingFingerprint else {
                        self.handleOpenFailure(
                            sessionID: sessionID,
                            url: url,
                            message: "The file changed while XMLEDITORX was loading its source text. Open it again after the other write finishes."
                        )
                        return
                    }
    ...
    private func activateSession(at i: Int) {
        ...
        let v = (Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String) ?? "?"
        window?.title = "\(s.url.lastPathComponent), XMLEDITORX v\(v)"
        updateWindowDocumentState()
        refreshTabStrip()
    ...
    func applyGlobalScale(_ scale: CGFloat) {
        ...
        UserDefaults.standard.set(Double(s), forKey: "XMLEDITORX.globalZoom")
    }
    ...
    private func togglePopOut(chrome: PaneChrome, glass: NSView, title: String) {
        ...
        win.setFrameAutosaveName("XMLEDITORXPopout-\(title)")
        win.delegate = self
    ...
    static let minimizedPanesKey = "XMLEDITORX.minimizedPaneTitles"
    ...
    static let tourShownKey = "XMLEDITORX.tourShown"
    ...
    static let askOnCloseKey = "XMLEDITORX.askOnClose"
    ...
    static let lastOpenFilesKey = "XMLEDITORX.lastOpenFiles"
    static let reopenLastFilesKey = "XMLEDITORX.reopenLastFiles"
    ...
    func prepareForApplicationTermination(completion: @escaping (Bool) -> Void) {
        ...
        if activeOpenRequest != nil || !savingSessionIDs.isEmpty {
            ...
            alert.informativeText = "Wait for loading or saving to finish, then close XMLEDITORX again."
            alert.addButton(withTitle: "OK")
            alert.runModal()
            completion(false)
            return
    ...
    static let closeChoiceKey = "XMLEDITORX.closeWindowChoice"   // "tab" | "all"
    ...
    static let workspaceModeKey = "XMLEDITORX.workspaceMode"
    ...
    static let popoutsFloatKey = "XMLEDITORX.popoutsFloat"
    ...
    static let treeOnRightKey     = "XMLEDITORX.layout.treeOnRight"
    static let inspectorOnLeftKey = "XMLEDITORX.layout.inspectorOnLeft"
    static let hierarchyOnTopKey  = "XMLEDITORX.layout.hierarchyOnTop"
...
extension MainWindowController {
    ...
    @objc func sharePrintSelection(_ sender: Any?) {
        ...
        runPrint(text, jobTitle: "Selection, \(currentFileURL?.lastPathComponent ?? "XMLEDITORX")")
    }
    ...
    /// reject it. XMLEDITORX never truncates without the user's confirmation.
    private func shareXMLText(
        ...
        XMLEDITORX's 128 KiB compatibility excerpt is not a macOS maximum. It will include \(formattedExactBytes(plan.includedUTF8Bytes)) and omit \(formattedExactBytes(plan.omittedUTF8Bytes)), ending at a complete Unicode character.
    ...
    private func xmlSharePrompt(
        ...
        if let excerpt, excerpt.isTruncated {
            disclosure = "\n\nXMLEDITORX compatibility excerpt: first \(formattedExactBytes(excerpt.includedUTF8Bytes)) of \(formattedExactBytes(excerpt.totalUTF8Bytes)); \(formattedExactBytes(excerpt.omittedUTF8Bytes)) omitted. The XML may end before its closing tags."
        } else {
            disclosure = ""
        }
    ...
    @objc func shareCopySelectionInFull(_ sender: Any?) {
        ...
        statusLabel.stringValue = "Copied the complete selection (\(formattedExactBytes(text.utf8.count))); XMLEDITORX applied no limit"
    }
